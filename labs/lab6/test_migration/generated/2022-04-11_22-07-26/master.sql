...
spool "test_migration_&filename..log"

-- Password file execution
@passworddefinition.sql

PROMPT Dropping Role ROLE_test_migration ...
DROP ROLE ROLE_test_migration ;
PROMPT Creating Role ROLE_test_migration ...
CREATE ROLE ROLE_test_migration ;

-- PROMPT Drop DBLab6 user
-- drop user DBLab6 cascade;
   
PROMPT Create user DBLab6
CREATE USER DBLab6 IDENTIFIED BY &&DBLab6_password PASSWORD EXPIRE ACCOUNT LOCK /* DEFAULT TABLESPACE USERS  TEMPORARY TABLESPACE TEMP */;
GRANT CREATE SESSION, RESOURCE, CREATE VIEW, CREATE MATERIALIZED VIEW, CREATE SYNONYM, UNLIMITED TABLESPACE TO DBLab6;
...
GRANT ALL ON utils TO ROLE_test_migration;
grant ALL on utils to DBLab6 with grant option;

set define on
prompt connecting to DBLab6
...
CREATE TABLE Employee (
  id_employee NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  name_employee NVARCHAR2(40) NOT NULL,
  lastname_employee NVARCHAR2(20) NOT NULL
);


PROMPT Creating Unique Constraint UQ__Employee__4D8AEEF3B4A7946B on table Employee
ALTER TABLE Employee
ADD CONSTRAINT UQ__Employee__4D8AEEF3B4A7946B UNIQUE (
  name_employee
)
ENABLE
;
PROMPT Creating Primary Key Constraint PK__Employee__F807679CFF2BC2F8 on table Employee ... 
ALTER TABLE Employee
ADD CONSTRAINT PK__Employee__F807679CFF2BC2F8 PRIMARY KEY
(
  id_employee
)
ENABLE
...
GRANT ALL ON Employee TO ROLE_test_migration;
...
PROMPT Creating Primary Key Constraint orders_products_pk on table Orders_products ... 
ALTER TABLE Orders_products
ADD CONSTRAINT orders_products_pk PRIMARY KEY
(
  order_id,
  product_id
)
ENABLE
;
PROMPT Creating Check Constraint CK__Orders_pr__produ__30F848ED on table Orders_products... 
ALTER TABLE Orders_products
ADD CONSTRAINT CK__Orders_pr__produ__30F848ED CHECK 
(
   ( product_count > (0) )
) 
ENABLE
...
GRANT ALL ON Orders_products TO ROLE_test_migration;
-- DROP TABLE Orders CASCADE CONSTRAINTS;


PROMPT Creating Table Orders ...
CREATE TABLE Orders (
  id_order NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  order_employee_id NUMBER(10,0),
  order_client_id NUMBER(10,0),
  order_date DATE,
  all_price FLOAT
);


PROMPT Creating Primary Key Constraint PK__Orders__DD5B8F3F5F30D060 on table Orders ... 
ALTER TABLE Orders
ADD CONSTRAINT PK__Orders__DD5B8F3F5F30D060 PRIMARY KEY
(
  id_order
)
ENABLE
...
GRANT ALL ON Orders TO ROLE_test_migration;
-- DROP TABLE Products CASCADE CONSTRAINTS;


PROMPT Creating Table Products ...
CREATE TABLE Products (
  id_product NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  product_name NVARCHAR2(20) NOT NULL,
  price FLOAT NOT NULL
);


PROMPT Creating Primary Key Constraint PK__Products__BA39E84FFBBE9F05 on table Products ... 
ALTER TABLE Products
ADD CONSTRAINT PK__Products__BA39E84FFBBE9F05 PRIMARY KEY
(
  id_product
)
ENABLE
...
GRANT ALL ON Products TO ROLE_test_migration;
-- DROP TABLE Clients CASCADE CONSTRAINTS;


PROMPT Creating Table Clients ...
CREATE TABLE Clients (
  id_client NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  phone NVARCHAR2(20),
  adress NVARCHAR2(20) NOT NULL
);


PROMPT Creating Primary Key Constraint PK__Clients__6EC2B6C0903D4022 on table Clients ... 
ALTER TABLE Clients
ADD CONSTRAINT PK__Clients__6EC2B6C0903D4022 PRIMARY KEY
(
  id_client
)
ENABLE
...
GRANT ALL ON Clients TO ROLE_test_migration;
PROMPT Creating Index findClient on Clients ...
CREATE INDEX findClient ON Clients
(
  phone
) 
;
...
alter session set current_schema=DBLab6;
set define off
CREATE OR REPLACE FORCE VIEW All_Orders 
AS 
   SELECT * 
     FROM Orders ;


GRANT ALL ON All_Orders TO ROLE_test_migration;
CREATE OR REPLACE PROCEDURE addNewEmployee
(
  v_name IN NVARCHAR2,
  v_lastname IN NVARCHAR2
)
AS
...
BEGIN
   ...
   INSERT INTO Employee
     ...
     VALUES ( v_name, v_lastname );

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON addNewEmployee TO ROLE_test_migration;
CREATE OR REPLACE PROCEDURE addProductinOrder
(
  v_order_id IN NUMBER,
  v_product_id IN NUMBER,
  v_product_count IN NUMBER
)
AS
...
BEGIN
   ...
   INSERT INTO Orders_products
     ...
     VALUES ( v_order_id, v_product_id, v_product_count );

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON addProductinOrder TO ROLE_test_migration;
CREATE OR REPLACE PROCEDURE delClient
(
  v_idclient IN NUMBER
)
AS
...
BEGIN
   ...
   DELETE Clients
    ...
    WHERE  id_client = v_idclient;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON delClient TO ROLE_test_migration;
...
END;
/
GRANT ALL ON deleteEmployee TO ROLE_test_migration;
...
END;
/
GRANT ALL ON getAllClients TO ROLE_test_migration;
...
END;
/
GRANT ALL ON getAllEmployee TO ROLE_test_migration;
...
END;
/
GRANT ALL ON getAllOrders TO ROLE_test_migration;
...
END;
/
GRANT ALL ON getAllProducts TO ROLE_test_migration;
...
BEGIN
   ...
   INSERT INTO Clients
     ( phone, adress )
     VALUES ( v_phone, v_adress );

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON newClient TO ROLE_test_migration;
CREATE OR REPLACE PROCEDURE newOrder
(
  v_order_employee_id IN NUMBER,
  v_order_client_id IN NUMBER
)
AS
   v_order_date DATE;
...
BEGIN
   ...
   INSERT INTO Orders
     ( order_employee_id, order_client_id, order_date, all_price )
     VALUES ( v_order_employee_id, v_order_client_id, v_order_date, 0 );

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON newOrder TO ROLE_test_migration;
CREATE OR REPLACE PROCEDURE spisok_orders
(
  v_datestart IN DATE,
  v_dateend IN DATE
)
...
BEGIN
   ...
   OPEN  v_cursor FOR
      SELECT * 
        FROM Orders 
       WHERE  order_date BETWEEN v_datestart AND v_dateend ;
      DBMS_SQL.RETURN_RESULT(v_cursor);

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON spisok_orders TO ROLE_test_migration;
...
END;
/
GRANT ALL ON updatePhone TO ROLE_test_migration;
CREATE OR REPLACE TRIGGER UpdateAllPrice CREATE TRIGGER UpdateAllPrice ON Orders_products AFTER INSERT AS UPDATE Orders SET all_price +=  --SQLDEV: NOT RECOGNIZED
   AFTER INSERT += . price * Orders_products . product_count FROM Orders_products INNER JOIN Orders ON Orders_products . order_id = Orders . id_order INNER JOIN Products ON Orders_products . product_id = Products . id_product ;  --SQLDEV: NOT RECOGNIZED
   ON Orders_products
   FOR EACH ROW
...
alter session set current_schema=DBLab6;
set define off
PROMPT Creating Foreign Key Constraint FK_order_client on table Clients...
ALTER TABLE Orders
ADD CONSTRAINT FK_order_client FOREIGN KEY
(
  order_client_id
)
REFERENCES Clients
...
PROMPT Creating Foreign Key Constraint FK_order_employee on table Employee...
ALTER TABLE Orders
...
PROMPT Creating Foreign Key Constraint FK__Orders_pr__produ__300424B4 on table Products...
ALTER TABLE Orders_products
ADD CONSTRAINT FK__Orders_pr__produ__300424B4 FOREIGN KEY
(
  product_id
)
REFERENCES Products
...
PROMPT Creating Foreign Key Constraint FK__Orders_pr__order__2F10007B on table Orders...
ALTER TABLE Orders_products
ADD CONSTRAINT FK__Orders_pr__order__2F10007B FOREIGN KEY
(
  order_id
)
REFERENCES Orders
